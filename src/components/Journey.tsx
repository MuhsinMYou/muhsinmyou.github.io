    ...
    <section id="journey" className="relative py-32 px-6 md:px-12 overflow-hidden">
      <div className="max-w-5xl mx-auto relative z-10">
        <div className="mb-24 text-center">
          <h2 className="text-sm font-mono text-secondary tracking-widest uppercase mb-2">My Path</h2>
          <h3 className="text-4xl md:text-5xl font-display font-bold text-white">The <span className="text-primary text-glow">Journey</span></h3>
        </div>
    ...
    <div className={`relative flex flex-col md:flex-row ${isLeft ? 'md:flex-row-reverse' : ''} md:justify-between items-start md:items-center w-full group`}>
      ...
      >
        <div className="flex flex-wrap items-center justify-between mb-3 gap-4">
          <span className="text-5xl font-bold text-white/10 group-hover:text-white/20 transition-colors font-display">{item.year}</span>
          {item.company && <span className="text-sm font-mono text-secondary px-3 py-1 rounded-full bg-secondary/10 border border-secondary/20">{item.company}</span>}
        </div>
